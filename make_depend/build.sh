...
CURL_VERSION=7.21.7
LIBEVENT_VERSION="2.0.12-stable"
ZLIB_VERSION=1.2.5
...
function do_curl {
	...
	./configure --prefix="${BUILD_DIR}" ${COMMON_OPTIONS} --with-random=/dev/urandom --with-ssl --with-zlib
...
function do_libevent {
	...
	if [ ! -e "${PACKAGE_NAME}.tar.gz" ]
	then
	  /usr/bin/curl -O "http://monkey.org/~provos/${PACKAGE_NAME}.tar.gz"
	fi
...
function do_transmission {	
	...
	then
	  /usr/bin/curl -O "http://download.m0k.org/transmission/files/${PACKAGE_NAME}.tar.bz2"
	fi
